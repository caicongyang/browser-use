...
async def run_ui_test():
    ...
    try:
        ...
        try:
            ...
            # 使用缓存获取页面元素 
            cached_elements = await context.get_current_page.get_elements_with_cache(current_url, force_refresh=True)
        ...
        try:
            ...
            # 输入用户名 - 不使用TypeAction，而是直接使用execute_action
            await controller.registry.execute_action("type", {"index": username_index, "text": "13600805241"}, context)
            logger.info("用户名输入完成")
        ...
        try:
            ...
            # 输入密码 - 使用字典形式的参数
            await controller.registry.execute_action("type", {"index": password_index, "text": "Aa123456"}, context)
            logger.info("密码输入完成")
        ...
        try:
            ...
            page = await context.get_current_page()
            current_url = page.url
            cached_elements = await context.get_elements_with_cache(current_url)
        ...
        try:
            ...
            page = await context.get_current_page()
            current_url = page.url
            cached_elements = await context.get_elements_with_cache(current_url, force_refresh=True)
