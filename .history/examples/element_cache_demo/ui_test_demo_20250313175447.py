...
async def input_text_to_element(controller, context, element_index, text):
    ...
    # 记录控制器中可用的操作
    available_actions = []
    if hasattr(controller.registry, '_registry'):
        available_actions = list(controller.registry._registry.keys())
    elif hasattr(controller.registry, 'registry'):
        available_actions = list(controller.registry.registry.keys())
    
    logger.info(f"可用的操作: {available_actions}")
    
    # 尝试多种可能的输入文本操作名称
    possible_actions = [
        "type", "input_text", "fill", "enter_text", "type_text", 
    ...
    for action in possible_actions:
        if action in available_actions:
            try:
                logger.info(f"尝试使用 '{action}' 操作输入文本")
                result = await controller.registry.execute_action(action, {"index": element_index, "text": text}, context)
                logger.info(f"使用 '{action}' 操作成功")
                return result
            except Exception as e:
                logger.warning(f"使用 '{action}' 操作失败: {str(e)}")
    
    # 如果没有找到匹配的操作，尝试直接输入文本
    try:
        # 获取元素并直接设置它的值
        logger.info("尝试直接设置元素值")
        ...
        if element and hasattr(element, 'selector'):
            ...
            logger.info("使用页面fill方法成功")
            return ActionResult(success=True, extracted_content=f"已输入文本: {text}")
        
        # 如果上面的方法不可用，尝试使用点击然后输入的方式
        if await controller.registry.execute_action("click_element", {"index": element_index}, context):
            logger.info("已点击元素，尝试输入文本")
            await page.keyboard.type(text)
            logger.info("使用页面keyboard.type方法成功")
            return ActionResult(success=True, extracted_content=f"已点击并输入文本: {text}")
        
    except Exception as e:
        logger.error(f"所有输入文本方法都失败: {str(e)}")
...
async def run_ui_test():
    ...
    # 显示控制器中注册的所有操作
    if hasattr(controller.registry, '_registry'):
        logger.info(f"控制器注册的操作: {list(controller.registry._registry.keys())}")
    elif hasattr(controller.registry, 'registry'):
        logger.info(f"控制器注册的操作: {list(controller.registry.registry.keys())}")
