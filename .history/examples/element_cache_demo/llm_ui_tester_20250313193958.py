...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        help='要执行的测试任务描述',
        default='访问https://hy-sit.1233s2b.com，登录系统，查找并验证关键页面元素'
    )
    parser.add_argument(
        '--provider',
        type=str,
...
class EnhancedUITestAgent:
    ...
    async def setup(self):
        ...
        if self.use_cache:
            ...
            self.context = extend_browser_context(self.context, cache_dir=self.cache_dir)
        
        # 将上下文添加到代理
        self.agent.controller.context = self.context
        
        # 绑定增强的UI操作方法
        self._bind_enhanced_methods()
        
    def _bind_enhanced_methods(self):
        """将增强的UI操作方法绑定到控制器和代理"""
        # 可以在这里添加自定义操作到控制器
        # 例如，添加一个input_text操作
        if not hasattr(self.controller.registry, "_enhanced_methods_bound"):
            logger.info("绑定增强的UI操作方法到控制器")
            
            # 可以在这里添加自定义操作到控制器的注册表
            # 这里只是标记已绑定，实际操作在执行时通过辅助函数完成
            self.controller.registry._enhanced_methods_bound = True
