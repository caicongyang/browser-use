...
from examples.element_cache_demo.browser_extension.context_extension import extend_browser_context
...
async def run_demo():
    ...
    # 扩展浏览器上下文，添加缓存功能
    context = extend_browser_context(context, cache_dir="demo_cache")
...
async def test_cached_element_location(context, controller):
    ...
    # 获取DOM状态
    dom_state = await context.get_dom_state()
...
def display_sample_cache(cached_elements, sample_size=3):
    ...
    for key in sample_keys:
        ...
        logger.info(f"  元素 {key}: <{tag_name} {attr_str}> {text}")
